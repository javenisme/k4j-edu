...
log_level = os.getenv("LAMB_KB_LOG_LEVEL", os.getenv(
    "GLOBAL_LOG_LEVEL", "WARNING")).upper()
...
CHROMADB_PATH = os.getenv("CHROMADB_PATH", os.path.join(
...
CHROMADB_PATHS = [
    ...
    os.path.join(os.path.dirname(
        os.path.abspath(__file__)), "data", "chromadb"),
...
class LambKBClient:
        ...
        if data and isinstance(data, dict):
            ...
            for key, value in data.items():
                if isinstance(value, dict):
                    ...
                    data[key] = [self._dict_to_obj(item) if isinstance(
    ...
    def list_collections(self) -> List[Dict[str, Any]]:
        ...
        if collections and isinstance(collections, dict) and "items" in collections:
            collections["items"] = [self._dict_to_obj(
                item) for item in collections["items"]]
    ...
    def list_files(self, collection_id: int, status=None) -> List[Dict[str, Any]]:
        ...
        files = self._request(
            "get", f"/collections/{collection_id}/files", params=params)
    ...
    def query_collection(self, collection_id: int, query_text: str, top_k: int = 5,
        ...
        if isinstance(response, dict):
            ...
            if "timing" in response and not isinstance(response["timing"], dict):
                try:
                    ...
                    if isinstance(response["timing"], (int, float)):
                        response["timing"] = {
                            "total_seconds": response["timing"]}
                    else:
                        response["timing"] = {
                            "total_time": str(response["timing"])}
                except:
            ...
            logger.info(
                f"Pre-processed query response: count={response['count']}, results_length={len(response['results'])}")
        else:
            # If response is not a dict, create a valid structure
            logger.warning(
                f"Query response is not a dictionary: {type(response)}")
            response = {
                "results": [],
                "count": 0,
    ...
    def create_collection(self, collection_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'embeddings_model' in collection_data and 'vendor' in collection_data['embeddings_model']:
                ...
                safe_data = json.loads(json.dumps(
                    collection_data))  # Deep copy
                if 'apikey' in safe_data.get('embeddings_model', {}):
                    safe_data['embeddings_model']['apikey'] = '[REDACTED]'
                logger.info(
                    f"Creating collection with {vendor} embeddings using model {model}")
            ...
            if isinstance(response, dict) and "chromadb_uuid" in response:
                logger.info(
                    f"Collection created successfully with ChromaDB UUID: {response['chromadb_uuid']}")
...
class ChromaDBHelper:
    ...
    def __init__(self, db_paths: List[str]):
        ...
        for path in db_paths:
            try:
                ...
                logger.info(
                    f"Connected to ChromaDB at {path}, found {len(collections)} collections")
                self.db_path = path
                break
            except Exception as e:
        ...
        if self.client is None:
            logger.error(
                "Failed to connect to ChromaDB with any of the provided paths")
    ...
    def get_collection_details(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            embedding_dims = [len(
                emb) for emb in sample_docs["embeddings"]] if sample_docs["embeddings"] else []
            ...
            logger.error(
                f"Error getting collection details for {collection_name}: {e}")
            return {
                "name": collection_name,
                "error": str(e),
                "success": False
    ...
    def list_collections(self) -> List[str]:
        ...
        try:
            if self.client is None:
                logger.error(
                    "Cannot list collections: ChromaDB client is None")
                return []
            ...
            try:
                ...
                if collections and isinstance(collections[0], str):
                    ...
                    logger.info(
                        f"Found {len(collections)} collections in ChromaDB at {self.db_path}")
                    return collections
                else:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Found {len(collection_names)} collections in ChromaDB at {self.db_path}")
                        return collection_names
                    except (AttributeError, NotImplementedError):
                        logger.warning(
                            f"Unexpected collection format: {collections[0]}")
                        return []
    ...
    def get_document_stats(self, collection_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for meta in all_docs["metadatas"]:
                ...
                if "source" in meta:
                    source = meta["source"]
                    stats["by_source"][source] = stats["by_source"].get(
                        source, 0) + 1

                # Count by chunk unit
                if "chunk_unit" in meta:
                    unit = meta["chunk_unit"]
                    stats["by_chunk_unit"][unit] = stats["by_chunk_unit"].get(
                        unit, 0) + 1

                # Count by chunking strategy
                if "chunking_strategy" in meta:
                    strategy = meta["chunking_strategy"]
                    stats["by_chunking_strategy"][strategy] = stats["by_chunking_strategy"].get(
                        strategy, 0) + 1

                # Count presence of each metadata field
                for key in meta:
                    stats["by_metadata_presence"][key] = stats["by_metadata_presence"].get(
                        key, 0) + 1
    ...
    def get_advanced_diagnostics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            chromadb_api_collections = [
                {"name": name, "count": 0} for name in collection_names]
            ...
            logger.info(
                f"SQLite Collections: {[c['name'] for c in sqlite_collections]}")

            # Connect to ChromaDB SQLite database
            chromadb_internal_collections = self._get_chromadb_collections_from_sqlite()
            logger.info(
                f"ChromaDB Internal Collections: {[c['name'] for c in chromadb_internal_collections]}")
            ...
            for col in sqlite_collections:
                ...
                found_in_api = any(name.lower().strip() ==
                                   col_name for name in collection_names)
            ...
            for uuid_dir in uuid_dirs:
                ...
                if matching_internal_collection:
                    ...
                    collection_name = matching_internal_collection.get(
                        'name', '')
                    if collection_name:
    ...
    def _get_sqlite_collections(self) -> List[Dict[str, Any]]:
        ...
        sqlite_path = os.path.join(os.path.dirname(
            self.db_path), "lamb-kb-server.db")
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, name, owner, creation_date, embeddings_model FROM collections")
        ...
        for collection in collections:
            ...
            if isinstance(collection['embeddings_model'], str):
                try:
                    collection['embeddings_model'] = json.loads(
                        collection['embeddings_model'])
    ...
    def _get_chromadb_collections_from_sqlite(self) -> List[Dict[str, Any]]:
        ...
        if not os.path.exists(chroma_db_path):
            logger.error(
                f"ChromaDB SQLite file not found at: {chroma_db_path}")
            return []
        ...
        try:
            ...
            logger.info(
                f"Available columns in collections table: {available_columns}")
            ...
            if not query_columns:
                logger.warning(
                    "Required columns 'id' and 'name' not found in collections table")
                conn.close()
                return []
            ...
            for row in cursor.fetchall():
                ...
                if 'id' in collection_data:
                    try:
                        ...
                        logger.warning(
                            f"Invalid UUID format for collection {collection_data.get('name', 'unknown')}: {collection_data['id']}")

                collections.append(collection_data)

            # Log the results
            logger.info(
                f"Found {len(collections)} collections in ChromaDB SQLite")
            for col in collections:
                logger.info(
                    f"Collection from ChromaDB SQLite: name='{col.get('name', '')}', id='{col.get('id', '')}'")
                ...
                for collection in collections:
                    ...
                    try:
                        ...
                        metadata_cursor.execute(
                            "PRAGMA table_info(collection_metadata)")
                        metadata_columns = [row[1]
                        ...
                        if 'key' in metadata_columns and 'value' in metadata_columns:
                            ...
                            logger.warning(
                                f"Cannot fetch metadata - columns missing. Available: {metadata_columns}")
                            metadata = {}

                        collection['metadata'] = metadata
                    except Exception as e:
                        logger.error(
                            f"Error fetching metadata for collection {collection.get('id', 'unknown')}: {e}")
    ...
    try:
        if owner:
            ...
            if collections and 'items' in collections:
                collections['items'] = [
                    col for col in collections['items'] if col.get('owner') == owner]
        else:
            collections = client.list_collections()
        ...
        try:
            ...
            if embeddings_type == 'default':
                ...
                if vendor.lower() == 'openai' and not api_key:
                    api_key = "default"
                    logger.info(
                        "Using default OpenAI API key from environment")
            ...
            # Deep copy without shared references
            safe_data = json.loads(json.dumps(collection_data))
            if 'embeddings_model' in safe_data:
                # Replace API key for logging only
                if 'apikey' in safe_data['embeddings_model']:
                    safe_data['embeddings_model']['apikey'] = '[REDACTED]'
            logger.info(
                f"Creating collection '{safe_data['name']}' with {safe_data['embeddings_model']['vendor']} embeddings model {safe_data['embeddings_model']['model']}")
            ...
            if isinstance(new_collection, dict):
                ...
                logger.info(
                    f"Response fields: {', '.join(new_collection.keys())}")

                # Log the ChromaDB UUID if present
                if 'chromadb_uuid' in new_collection:
                    logger.info(
                        f"Collection created with ChromaDB UUID: {new_collection['chromadb_uuid']}")
            else:
                ...
                try:
                    ...
                    logger.info(
                        f"Response object attributes: {dir(new_collection)}")
                except Exception as attr_error:
                    logger.error(
                        f"Error processing response object: {attr_error}")

            # Successful creation - flash a message and redirect
            flash(
                f"Collection '{collection_name}' created successfully!", "success")

            try:
                # Try to redirect to the collection details page
                if collection_id:
                    logger.info(
                        f"Redirecting to collection ID: {collection_id}")
                    return redirect(url_for('view_collection', collection_id=collection_id))
                else:
                    # If no ID, try to find by name
                    logger.warning(
                        f"No ID in response, attempting to find collection by name: {collection_name}")
                    ...
                    if found and (isinstance(found, dict) and 'id' in found or hasattr(found, 'id')):
                        found_id = found.id if hasattr(
                            found, 'id') else found.get('id')
                        logger.info(
                            f"Found collection by name, ID: {found_id}")
...
def view_collection(collection_id):
    ...
    try:
        ...
        total_documents = sum(get_value(file, 'document_count', 0)
                              for file in files)
...
def ingest_file(collection_id):
    ...
    try:
        ...
        if file_ext not in {'txt', 'md', 'markdown', 'text'}:
            flash(
                f"Unsupported file type: .{file_ext}. Only .txt, .md, .markdown, and .text files are supported.", "error")
            return redirect(url_for('view_collection', collection_id=collection_id))
        ...
        flash(
            f"Successfully ingested file '{original_filename}' and created {documents_added} documents.", "success")
...
def query_collection(collection_id):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            include_all_metadata = request.form.get(
                'include_all_metadata') == 'on'
            ...
            try:
                ...
                logger.info(
                    f"Query results count: {results.count if hasattr(results, 'count') else 0}")
...
def debug_chromadb():
    ...
    try:
        ...
        chromadb_internal_collections = diagnostics.get(
            'chromadb_internal_collections', [])
        ...
        for api_col in api_collections:
            ...
            if not already_mapped:
                ...
                found_uuid = None
                for internal_col in chromadb_internal_collections:
                    internal_name = internal_col.get(
                        'name', '').lower().strip()
...
def debug_collection(collection_name):
    ...
    try:
        collection_details = chroma_helper.get_collection_details(
            collection_name)
...
def api_get_collection(collection_id):
    ...
    try:
        ...
        collection['total_documents'] = sum(
            file.get('document_count', 0) for file in files)
...
def advanced_diagnostics():
    ...
    try:
        ...
        api_collections = chroma_helper.list_collections()
        logger.info(
            f"Advanced Diagnostics Raw API Collections: {api_collections}")

        sqlite_collections = diagnostics_raw.get('sqlite_collections', [])
        chromadb_internal_collections = diagnostics_raw.get(
            'chromadb_internal_collections', [])

        # Print detailed debug info about collections
        logger.info(
            f"API Collections {len(api_collections)}: {api_collections}")
        logger.info(
            f"SQLite Collections {len(sqlite_collections)}: {[c['name'] for c in sqlite_collections]}")
        logger.info(
            f"ChromaDB Internal Collections {len(chromadb_internal_collections)}: {[(c.get('name', ''), c.get('id', '')) for c in chromadb_internal_collections]}")
        ...
        for col in sqlite_collections:
            ...
            logger.info(
                f"SQLite collection '{col['name']}': API match: {api_name}, UUID: {found_uuid}")
            ...
            mapping = {
                ...
                'found_in_api': found_in_api,
                'found_uuid': found_uuid is not None and found_uuid != '',
                # Add stored UUID from SQLite
                'stored_uuid': col.get('chromadb_uuid'),
        ...
        diagnostics['critical_mismatches'] = sum(
            1 for m in mismatches if m["severity"] == "high")
        diagnostics['medium_mismatches'] = sum(
            1 for m in mismatches if m["severity"] == "medium")
        diagnostics['minor_mismatches'] = sum(
            1 for m in mismatches if m["severity"] == "low")
...
if __name__ == '__main__':
    ...
    if chroma_helper.client:
        logger.info(
            f"Successfully connected to ChromaDB at: {chroma_helper.db_path}")
        collections = chroma_helper.list_collections()
        logger.info(
            f"Found {len(collections)} collections: {', '.join(collections)}")
    else:
