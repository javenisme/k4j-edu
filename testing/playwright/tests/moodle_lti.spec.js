...
test.describe.serial("Moodle LTI integration", () => {
  ...
  test("Clicking LTI activity redirects to OWI and shows assistant", async ({
    browser,
  }) => {
    ...
    const context = await browser.newContext({
      storageState: path.join(__dirname, ".auth", "moodle-state.json"),
    });
    const page = await context.newPage();

    // Visit the course page and find the activity link
    const courseUrl = COURSE_ID
    ...
    const assistantLocator = newPage
      .locator(`div:has-text("${EXPECTED_TEXT}")`)
      .first();
