...
async function ensureMoodleAuth() {
  if (!MOODLE_URL || !MOODLE_LOGIN || !MOODLE_PASSWORD) {
    throw new Error('MOODLE_URL, MOODLE_LOGIN and MOODLE_PASSWORD must be set in tests/.env');
  }
  ...
  const loginUrl = MOODLE_URL.endsWith('/') ? `${MOODLE_URL}login/index.php` : `${MOODLE_URL}/login/index.php`;
  ...
  if (!await page.$('#username')) {
    ...
    await page.waitForLoadState('domcontentloaded');
    const loginLink = await page.$("a:has-text('Log in')") || await page.$("a:has-text('Log In')");
    if (loginLink) {
  ...
  const usernameSelectors = ['#username', 'input[name="username"]', 'input[id*=user]', "input[placeholder*='Username']", "input[placeholder*='username']"];
  const passwordSelectors = ['#password', 'input[name="password"]', 'input[type="password"]', "input[placeholder*='Password']", "input[placeholder*='password']"];
  ...
  if (!usernameSel || !passwordSel) {
    ...
    for (const input of inputs) {
      ...
      if (!heuristicUser && /user|email|login/i.test(name + id + placeholder) && type !== 'password') {
        heuristicUser = input;
      }
    ...
    if (heuristicUser && heuristicPass) {
      ...
      try { fs.writeFileSync(debugPath, html); } catch (e) { /* ignore */ }
      await browser.close();
      throw new Error(`Could not find Moodle login fields (username/password). Page snapshot saved to ${debugPath}`);
    }
  } else {
  ...
  const buttonSelectors = ['#loginbtn', 'button[type="submit"]', 'button:has-text("Log in")', 'button:has-text("Login")'];
  let clicked = false;
  ...
  const successChecks = ["a[href*='logout']", "#action-menu-toggle", "a:has-text('Log out')", "a:has-text('Sign out')"];
  let success = false;
  ...
  if (!success) {
    ...
    console.warn('Login may have failed (logout/menu not found). Storage state saved anyway.');
  }
}
